  ...
  open: function(opts, entities) {
    ...
    var connectionResolver = function(resolve) {
      var createStructure = function() {
        var sqlStructure = "";
        if (opts.truncate) {
          sqlStructure += entities.map(function(entity) {
            return ormUtils.dropTableFromEntity(entity, entity.name);
          }).join("");
        }
        sqlStructure += entities.map(function(entity) {
          return ormUtils.createTableFromEntity(entity, entity.name);
        }).join("");
